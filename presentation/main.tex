...
\begin{frame}{Résumé des Gains de Performance}
    ...
    \begin{tabular}{lccccc}
        \toprule
        \textbf{Version} & \textbf{Thread/cell} & \textbf{Duration} (ms) & \textbf{Mem. BW} (GB/s) & \textbf{Mem. BW} (\%) & \textbf{FPS} \\
        \midrule
    ...
    \begin{tabular}{lcccc}
        \toprule
        \textbf{Version} & \textbf{L1/TEX Hit} (\%) & \textbf{Mem Busy} (\%) & \textbf{SM Busy} (\%) & \textbf{Compute} (\%) \\
        \midrule
...
\begin{frame}{Profiling: lookup4$\times$4}
    \centering
    \includegraphics[width=0.75\textwidth]{lookup4x4.png}
\end{frame}
