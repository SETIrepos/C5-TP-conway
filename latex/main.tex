...
\textbf{Performance obtenue :} \textbf{3208.89 FPS}, soit un gain de plus de 30\% par rapport à la version LUT précédente.
...
\textbf{Lookup Table + uint16} & \textbf{3208.89} \\
\bottomrule
\end{tabular}
\end{center}

\paragraph{Résultat mesuré}
La version actuelle du code (Lookup Table avec 1024 threads par bloc et bit-patching) atteint une performance mesurée de **FPS = 3208.89** sur la configuration matérielle indiquée. Cette valeur provient d'un benchmark réalisé avec la commande :
\begin{verbatim}
python conway.py profile --grid-size 4000 --iterations 1000
\end{verbatim}
